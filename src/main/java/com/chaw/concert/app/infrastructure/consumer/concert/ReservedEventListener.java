...
public class ReservedEventListener {
    ...
    public void saveOnDataPlatform(ReservedEvent reservedEvent) {
        kafkaProducer.sendMessage(ReserveKafkaTopics.CONCERT_RESERVE_TOPIC_DATASTORE, reservedEvent);
    }

    @Async
    @TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
    public void sendToSlack(ReservedEvent reservedEvent) {
        kafkaProducer.sendMessage(ReserveKafkaTopics.CONCERT_RESERVE_TOPIC_SLACK, reservedEvent);
    }
}
